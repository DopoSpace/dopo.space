<script lang="ts">
	...
	import { trackLoginSuccess, trackProfileFormStart, trackProfileFormSubmit } from '$lib/analytics';
	...
	let newsletterConsent = $derived(
		form?.values?.newsletterConsent ?? data.user.newsletterSubscribed ?? false
	);

	// Derived values for birth city/province/date fields
	let birthCity = $derived(
		birthCityOverride ?? form?.values?.birthCity ?? data.profile?.birthCity ?? ''
	);
	let birthProvince = $derived(
		birthProvinceOverride ?? form?.values?.birthProvince ?? data.profile?.birthProvince ?? ''
	);
	let birthDate = $derived(
		birthDateOverride ?? form?.values?.birthDate ?? formatDateForInput(data.profile?.birthDate)
	);

	// Derived values for firstName and lastName
	let firstName = $derived(
		firstNameOverride ?? form?.values?.firstName ?? data.profile?.firstName ?? ''
	);
	let lastName = $derived(
		lastNameOverride ?? form?.values?.lastName ?? data.profile?.lastName ?? ''
	);
	...
	let hasForeignTaxCode = $derived(
		hasForeignTaxCodeOverride ??
			form?.values?.hasForeignTaxCode ??
			data.profile?.hasForeignTaxCode ??
			false
	);
	let gender = $derived(genderOverride ?? form?.values?.gender ?? data.profile?.gender ?? '');
	...
	<div class="subscription-page">
		<TextContainer>
			...
			{:else if data.membershipState === SystemState.S1_PROFILE_COMPLETE}
				<p>
					{@html m
						.subscription_subtitle_profile_complete()
						.replace(
							'Procedi al pagamento',
							'<a href="/membership/checkout">' + m.subscription_proceed_payment() + '</a>'
						)}
				</p>
			...
			{#if data.membershipState === SystemState.S4_AWAITING_NUMBER}
				<p>
					{m.subscription_status_payment_completed_text()}. {m.subscription_status_payment_completed_note()}
				</p>
			...
			{:else if data.membershipState === SystemState.S3_PAYMENT_FAILED}
				<p>
					{m.subscription_status_failed_text()}.
					<a href="/membership/checkout">{m.subscription_status_failed_retry()}</a>
					{m.subscription_status_failed_note()}
				</p>
			{:else if data.membershipState === SystemState.S6_EXPIRED}
				<p>
					{m.subscription_status_expired_text()}.
					<a href="/membership/checkout">{m.subscription_status_expired_renew()}</a>
					{m.subscription_status_expired_note()}
				</p>
			{/if}
			...
			{#if data.profileComplete}
				...
				>
					...
					<div class="profile-toggle-content">
						...
						<span class="profile-toggle-value"
							>{data.profile?.firstName} {data.profile?.lastName}</span
						>
					</div>
					<div class="profile-toggle-action">
						<span class="profile-toggle-action-text"
							>{formExpanded ? m.common_close() : m.common_edit()}</span
						>
						<svg
				...
				>
					...
					<div class="form-section">
						...
						<div class="grid grid-cols-2">
							<div class="form-group">
								<label for="firstName"
									>{m.subscription_first_name()} <span class="required">*</span></label
								>
								<input
									type="text"
									id="firstName"
							...
							<div class="form-group">
								<label for="lastName"
									>{m.subscription_last_name()} <span class="required">*</span></label
								>
								<input
									type="text"
									id="lastName"
					...
					<div class="form-section">
						...
						{#if isUnder16}
							<div class="warning-box">
								...
								<p>
									{@html m.subscription_under16_text({
										email:
						...
						{#if isItalian}
							...
							<div class="grid grid-cols-3">
								...
								<div class="form-group">
									...
									<input type="text" id="birthProvinceDisplay" value={birthProvince} disabled />
								</div>
							</div>
						{:else if nationality}
					...
					<div class="form-section">
						...
						{#if showTaxCodeField}
							<div class="form-group">
								<label for="taxCode"
									>{m.subscription_tax_code()}
									{#if !isForeigner}<span class="required">*</span>{/if}</label
								>
								<input
									type="text"
									id="taxCode"
						...
						{#if isForeigner}
							<div class="checkbox-group">
								<input
									...
									value="true"
									onchange={(e) =>
										(hasForeignTaxCodeOverride = (e.target as HTMLInputElement).checked)}
									onblur={(e) =>
										handleCheckboxBlur('hasForeignTaxCode', (e.target as HTMLInputElement).checked)}
								/>
								<label for="hasForeignTaxCode" class="checkbox-label"
									>{m.subscription_has_foreign_tax_code()}</label
								>
							</div>
						...
						<div class="consent-item">
							<div class="checkbox-group">
								<input
									...
									value="true"
									onblur={(e) =>
										handleCheckboxBlur('privacyConsent', (e.target as HTMLInputElement).checked)}
						...
						<div class="consent-item">
							<div class="checkbox-group">
								<input
									...
									value="true"
									onblur={(e) =>
										handleCheckboxBlur('dataConsent', (e.target as HTMLInputElement).checked)}
